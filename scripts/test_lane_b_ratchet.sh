...
# lowercase-only test walks straight past it. Measured: with shardpilot/client.lua marked
...
# Linking shardpilot/client.lua or the baseline to a path outside this worktree
# leaves `git status --porcelain` clean, so the dirty-tree guard passes. The
# appends below then write THROUGH the inode and change every name for it, while
# the restore replaces only the worktree entry -- with a NEW inode, since
