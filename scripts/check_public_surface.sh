...
# and an internal project codename came to sit in a public repository for months. A
# gate that runs where the exposure already exists is the fix.
#
# It matters more here than in a source-only library: the documented Defold
# dependency URL is this repository's whole tag archive, so every consuming
# project downloads every tracked file, not just `shardpilot/`.
#
...
PATTERNS='ADR-[0-9]+|§[0-9]|[Tt]here is (no|NO) [A-Za-z]+ (harness|coverage|test|suite)|(is|are) not (tested|covered|scanned|audited|monitored)|no (automated )?(tests?|coverage|scanning|monitoring) (for|of|in)|nobody (looks|checks|monitors)|GAP-[0-9]{3}|\bSP-[0-9]{3}\b|\bAC-[A-Z]{2}-[0-9]+|analytics[-_ ]service|control[-_ ]plane|shardpilot/(docs|qa|infra|integrations)|[Pp]roject[-_ ][Tt]ower|shepherd-pr|verification-discipline|Codex (review|#|[a-z]+#)|[A-Z][A-Z0-9]*(_[A-Z0-9]+)+_(ENABLED|DISABLED|MODE)|\b(main|master|HEAD) @ *`?[0-9a-f]{7,40}'
...
EOF
  ...
  (
    ...
    printf '%s\n' '-- GAP-075 note' 'return {}' > lane_b.lua
...
echo
echo "LANE B (REPORTED, NOT GATED) — Lua source: ${scan_lane_b_files} file(s), ${scan_lane_b_lines} matching line(s)."
if [ "$scan_lane_b_files" -eq 0 ]; then
  ...
  echo "  These comments ship inside the library every consuming project pulls in."
  echo "  They are owed work, not accepted risk, and are not gated HERE because this"
  echo "  repository's Lua sources are owned by the SDK wire-freeze workstream. The"
  echo "  count is lines"
