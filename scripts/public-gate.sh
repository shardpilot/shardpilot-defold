...
with tempfile.TemporaryDirectory(prefix='public-gate-scenes-') as directory:
    ...
    baseline_file = repo / 'scripts/public-surface-lane-b-baseline.txt'
    original_baseline = baseline_file.read_text()
    ...
    total = 0
    finding_scenes = {'completed-finding', 'baseline-mismatch', 'target-new-file', 'target-raised-count'}
    for name in ['clean-zero-comparison', 'completed-finding', 'baseline-mismatch', 'target-new-file', 'target-raised-count', 'stopped-after-finding', 'dropped-tree-read', 'killed-reader', 'short-tree-walk', 'stale-startup', 'helper-mismatch', 'channel-write-failure', 'baseline-write', 'stdout-closes-after-json', 'channel-closes-after-json']:
        if selected and name not in selected: continue
        call(['git', 'reset', '--soft', base_head], repo)
        gate.write_text(original)
        helper.write_text(original_helper)
        baseline_file.write_text(original_baseline)
        planted = repo / 'receipt-scene.txt'
        if planted.exists(): planted.unlink()
        if name == 'completed-finding': planted.write_text('See ' + 'ADR-' + '0' * 4 + '.\n')
        if name in {'baseline-mismatch', 'target-new-file', 'target-raised-count'}:
            row = next(s for s in original_baseline.splitlines() if s and not s.startswith('#'))
            count, path = row.split(' ', 1)
            replacement = '' if name == 'target-new-file' else str(int(count) + (1 if name == 'baseline-mismatch' else -1)) + ' ' + path + '\n'
            baseline_file.write_text(original_baseline.replace(row + '\n', replacement, 1))
            if name != 'baseline-mismatch':
                call(['git', 'add', str(baseline_file)], repo)
                call(['git', '-c', 'user.name=Synthetic', '-c', 'user.email=fixture@example.invalid', 'commit', '-qm', 'Synthetic comparison target'], repo)
                baseline_file.write_text(original_baseline)
...
'''.replace('@ACTION@', action) + permission_model)
        ...
        if name not in {'clean-zero-comparison', 'baseline-mismatch'}: env['PUBLIC_SURFACE_BASE_REF'] = 'HEAD'
        ...
        if name in finding_scenes or name in ['clean-zero-comparison', 'baseline-write']:
            assert complete(r), (name, run.returncode, r, run.stdout[-1000:], run.stderr[-1000:])
            assert run.returncode == (1 if name in finding_scenes else 0), (name, run.returncode)
            assert r['findings'] == (1 if name in finding_scenes else 0), (name, r)
            if name in finding_scenes:
                expected_reason = {'completed-finding': 'internal material in the published non-source surface', 'baseline-mismatch': 'moved and the baseline did not agree', 'target-new-file': 'is absent from', 'target-raised-count': 'the baseline was raised'}[name]
                assert expected_reason in run.stderr, (name, run.stderr)
