...
-- The player's answer and the notice it was given against. `pending` means the
...
local function record(fresh)
  ...
  -- recorded, which is the ordering the durable consent outbox exists to
  -- prevent.
  local recorded, consent_err = shardpilot.set_consent(answered.granted)
  if not recorded then
    print("shardpilot consent not recorded: " .. tostring(consent_err) .. "; owed")
    return -- `answered.pending` stays true; retry on the next trigger
  end
  answered.pending = false
  if answered.granted then
    shardpilot.session_start()
  end
end

local function start(fresh)
...
local resolve_after_answer

-- Present the notice for THIS decision, then act only on a freshly resolved
-- one.
local function present(decision)
  present_your_consent_notice(decision, function(granted)
    -- ⚠ STORE THE ANSWER BEFORE ANYTHING CAN FAIL, with the notice it was
    -- given against. A refusal further down must not lose what the player said.
    answered = { text_version = decision.consent_text_version,
                 language = decision.presented_language,
                 granted = granted, pending = true }
    resolve_after_answer(decision)
  end)
end

resolve_after_answer = function(decision)
  -- ⚠ RE-RESOLVE BEFORE ACTING ON THE ANSWER, and invalidate first: the player
  -- was reading the screen, the plan may have expired or the policy may have
  -- been revoked meanwhile, and without the invalidation this resolution is
  -- answered by the private cache entry the first call wrote — the very
