...
# name and an internal project codename came to sit in a public repository for
# months. A gate that runs where the exposure already exists is the fix.
#
# It also matters more here than in a source-only library: the documented
# Defold dependency URL is this repository's whole tag archive, so every
# consuming project downloads every tracked file, not just `shardpilot/`.
#
# ── SCOPE, STATED SO A PASS IS NOT READ AS MORE THAN IT IS ───────────────────
# LANE A is GATED at zero: every tracked file that is not Lua source.
# LANE B is REPORTED, NOT GATED: Lua source (*.lua). Those comments are owned
#   by another workstream (the SDK wire freeze) and editing them here would
...
the analytics-service schema-revision handshake
...
echo
echo "LANE B (REPORTED, NOT GATED) — Lua source: ${lane_b_files} file(s), ${lane_b_hits} line(s)."
if [ "$lane_b_files" -eq 0 ]; then
  ...
  echo "  They are owed work, not accepted risk, and they are not gated HERE because"
  echo "  this repository's Lua sources are owned by the SDK wire-freeze workstream."
fi
