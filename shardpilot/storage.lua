...
--
-- One durable per-app record for the experiment-assignment client (GAP-017):
-- a small list of last-known-good assignment records, each holding the raw
-- response body plus the (app, environment, experiment, subject, url) scope
-- string and the experiment key it was fetched for. The scope check itself
-- lives in the assignment client; this store only persists and validates the
-- record shape. Like the remote-config cache it is best-effort — a failed
-- write costs only offline serving, never the fetched decision — and unlike
-- the spool a failed write never evicts (an evict-and-retry could silently
-- drop another experiment's record while reporting success).

local experiment_assignments_memory = {}

-- Assignment bodies are small fixed-shape JSON plus an author-defined
-- variant payload; one record is refused above this bound so the whole list
-- (16 records max) stays far under Defold's documented ~512 KB sys.save cap.
local max_experiment_assignment_body_bytes = 16384
local max_experiment_assignment_records = 16

local function experiment_assignment_entry(record)
	return {
		scope = record.scope,
...
local function valid_experiment_assignment_entry(entry)
	return type(entry) == "table"
		and type(entry.scope) == "string" and entry.scope ~= ""
		and type(entry.experiment_key) == "string" and entry.experiment_key ~= ""
		and type(entry.body) == "string" and entry.body ~= ""
		and #entry.body <= max_experiment_assignment_body_bytes
end

-- Keep only well-formed records, copied down to the known fields. A corrupt
-- or garbled record degrades to the salvageable subset instead of erroring
-- into game code.
local function sanitize_experiment_assignments(records)
	local out = {}
	if type(records) ~= "table" then
		return out
	end
	for i = 1, #records do
		if valid_experiment_assignment_entry(records[i]) then
			out[#out + 1] = experiment_assignment_entry(records[i])
		end
	end
	return out
end

local function write_experiment_assignments(ns, records)
	local record = { records = records }
	local path = save_path(ns, "experiment-assignments")
	if not path then
		-- No durable backend (plain Lua host): the in-memory record is the
		-- store.
		experiment_assignments_memory[ns] = record
		return true
	end
	local ok, saved = pcall(sys.save, path, record)
	if not (ok and saved == true) then
...
-- Load the cached assignment records for this app (possibly empty). The
-- cache shares the per-app namespace scheme of the spool and the
-- remote-config cache. A failed or garbled read degrades to the salvageable
-- subset; never throws.
function M.load_experiment_assignments(scope)
	local ns = spool_namespace(scope)
	local record = nil
	local path = save_path(ns, "experiment-assignments")
	if path then
	...
	if record == nil then
		record = experiment_assignments_memory[ns]
	end
	if type(record) ~= "table" then
		return {}
	end
	return sanitize_experiment_assignments(record.records)
end

-- Replace-or-append one assignment record (`{ scope, experiment_key, body,
-- fetched_at_ms }`), keyed by its scope string. The list is bounded: when a
-- fresh scope would exceed the record cap, the OLDEST record (by fetch
-- stamp) is evicted first. Returns true when stored — durably, or in the
-- in-memory fallback on hosts without the save-file API.
function M.save_experiment_assignment(scope, record)
	if not valid_experiment_assignment_entry(record) then
		return false
	end
	local ns = spool_namespace(scope)
	local records = M.load_experiment_assignments(scope)
	local stored = experiment_assignment_entry(record)
	local replaced = false
	for i = 1, #records do
		if records[i].scope == stored.scope then
			records[i] = stored
			replaced = true
			break
		end
	...
	if not replaced then
		records[#records + 1] = stored
	end
	while #records > max_experiment_assignment_records do
		-- Evict the oldest record (by fetch stamp) that is NOT the record
		-- being saved — persisting the new record is the whole point. With a
		-- cap above one, an evictable record always exists while over it.
		local oldest = nil
		for i = 1, #records do
			if records[i].scope ~= stored.scope
				and (not oldest or records[i].fetched_at_ms < records[oldest].fetched_at_ms) then
				oldest = i
			end
		end
		if not oldest then
		...
		table.remove(records, oldest)
	end
	return write_experiment_assignments(ns, records)
end

-- Drop the cached assignment record for ONE scope, leaving every other
-- record in place — the durable half of the sentinel-keyed cache drop (a
-- surface flipped off must not keep serving a cached assignment on the next
-- launch). Clearing an absent scope is an idempotent no-op. Returns true
-- when the rewrite landed.
function M.clear_experiment_assignment(scope, scope_string)
	if type(scope_string) ~= "string" or scope_string == "" then
		return false
	end
	local ns = spool_namespace(scope)
	local records = M.load_experiment_assignments(scope)
	local kept = {}
	for i = 1, #records do
		if records[i].scope ~= scope_string then
			kept[#kept + 1] = records[i]
		end
	end
	return write_experiment_assignments(ns, kept)
end
